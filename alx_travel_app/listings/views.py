    ...
    def verify_payment(self, request, pk=None):
        ...
        try:
            ...
            if response.status_code == 200 and response_data.get('status') == 'success':
                ...
                # Enhanced error handling and logging for Celery task
                try:
                    logger.info(f"Attempting to queue confirmation email for booking {booking.id}")
                    task = send_booking_confirmation_email.apply_async(
                        kwargs={
                        ...
                        countdown=1,  # Small delay to ensure transaction is complete
                        retry=True,
                        retry_policy={
                            'max_retries': 3,
                            'interval_start': 0,
                    ...
                    logger.error(f"Unexpected error in email task: {str(e)}", exc_info=True)
